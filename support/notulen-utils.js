...
import { fetchParticipationList, buildParticipantCache } from "./query-utils.js";
...
export function constructHtmlForMeetingNotesFromData({
  meeting,
  agendapoints,
  treatmentsData,
  intermissions,
...
export async function ensureVersionedNotulen(
  meeting,
  treatments,
  kind,
  publicTreatments = []
...
export async function signVersionedNotulen(
  versionedNotulenUri,
  sessionId,
  targetStatus,
  attachments
...
export async function publishVersionedNotulen(
  versionedNotulenUri,
  sessionId,
  targetStatus,
  attachments
...
async function updateLinkedDocuments(versionedNotulenUri) {
  await query(`
    ...
    DELETE {
      ...
      ?container ext:editorDocumentStatus ${sparqlEscapeUri(
        DRAFT_DECISON_PUBLISHED_STATUS
      )}
    } WHERE {
