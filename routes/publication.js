 ...
 */
router.post(
  '/signing/agenda/publish/:agendaKindUuid/:meetingUuid',
  async function (req, res, next) {
    try {
      ...
      const error = new Error(
        `An error occurred while publishing the agenda ${req.params.meetingUuid}: ${err}`
      );
      return next(error);
    }
  }
 ...
 */
router.post(
  '/signing/besluitenlijst/publish/:zittingIdentifier',
  async function (req, res, next) {
    ...
    try {
      ...
      console.log(err);
      const error = new Error(
        `An error occurred while publishing the besluitenlijst ${req.params.zittingIdentifier}: ${err}`
      );
      return next(error);
    }
    try {
      ...
      await publishVersionedBesluitenlijst(
...
router.post(
  ...
  async function (req, res, next) {
    try {
      ...
      console.log(err);
      const error = new Error(
        `An error occurred while publishing the behandeling ${req.params.behandelingUuid}: ${err}`
      );
      return next(error);
    }
  }
 ...
 */
router.post(
  '/signing/notulen/publish/:zittingIdentifier',
  async function (req, res, next) {
    let publishingTask;
    let meetingUuid;
    let meeting;
    try {
      ...
      console.log(err);
      const error = new Error(
        `An error occurred while publishing the notulen ${req.params.zittingIdentifier}:${err}`
      );
      return next(error);
    }
    try {
        ...
        for (const treatment of treatments) {
          if (publicBehandelingUris.includes(treatment.uri)) {
            ...
            if (!published) {
              ...
              const linkedRegulatoryStatementContainers =
                await getLinkedDocuments(treatmentEditorDocumentUri);
              const linkedRegulatoryStatementDocuments = await Promise.all(
                linkedRegulatoryStatementContainers.map(async (containerURI) =>
                  getCurrentVersion(containerURI)
                )
              ...
              await Promise.all(
                versionedRegulatoryStatements.map(
                  async (versionedStatementUri) =>
                    publishVersionedRegulatoryStatement(
                      versionedStatementUri,
                      req.header('MU-SESSION-ID'),
                      'gepubliceerd'
